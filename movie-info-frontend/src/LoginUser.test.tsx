...
describe("LoginUser", async () => {
  describe("When using good login info", async () => {
    it("Should log in user successfully", async () => {
      render(<MemoryRouter><LoginUser /></MemoryRouter>);  // MemoryRouter required to use navigate()
  ...
  describe("When using a bad username for login", () => {
    it("Should show error 'Login failed. Please check your credentials and try again.'", async () => {
      ...
      const loginButton = screen.getByRole("button", { name: "login" });
      await fireEvent.click(loginButton);
      const loginFailed = await screen.findByText("Login failed. Please check your credentials and try again.");
