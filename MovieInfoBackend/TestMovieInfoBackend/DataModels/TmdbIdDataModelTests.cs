...
public class TmdbIdDataModelTests
{
    ...
    public void GetModelFromResponse_ValidResponse_ReturnsValidModels()
    {
        ...
        {  // Movie  (blocks help prevent data leakage between tests)
            ...
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.BackdropPath), "Movie id BackdropPath must not be empty");
            Assert.True(movieIdDataModel.TmdbId > 0, "Movie id TmdbId must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.Title), "Movie id Title must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.OriginalTitle), "Movie id OriginalTitle must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.Overview), "Movie id Overview must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.PosterPath), "Movie id PosterPath must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.MediaType), "Movie id MediaType must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(movieIdDataModel.OriginalLanguage), "Movie id OriginalLanguage must not be empty");
            Assert.NotEmpty(movieIdDataModel.GenreIds);
            Assert.True(movieIdDataModel.Popularity >= 0.0, "Movie id Popularity must not be a negative decimal number");
            Assert.True(movieIdDataModel.VoteAverage >= 0.0, "Movie id VoteAverage must not be a negative decimal number");
            Assert.True(movieIdDataModel.VoteCount >= 0, "Movie id VoteCount must not be a negative number");
        ...
        {  // Person
            ...
            Assert.True(personIdDataModel.TmdbId > 0, "Person id TmdbId must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(personIdDataModel.Name), "Person id Name must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(personIdDataModel.OriginalName), "Person id OriginalName must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(personIdDataModel.MediaType), "Person id MediaType must not be empty");
            Assert.True(personIdDataModel.Popularity >= 0.0, "Person id Popularity must not be a negative decimal number");
            Assert.True(personIdDataModel.Gender >= 0, "Person id Gender must not be a negative number");
            Assert.False(String.IsNullOrWhiteSpace(personIdDataModel.KnownForDepartment), "Person id KnownForDepartment must not be empty");
        ...
        {  // TV Series
            ...
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.BackdropPath), "TV series id BackdropPath must not be empty");
            Assert.True(tvSeriesIdDataModel.TmdbId > 0, "TV series id TmdbId must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.Name), "TV series id Name must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.OriginalName), "TV series id OriginalName must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.Overview), "TV series id Overview must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.PosterPath), "TV series id PosterPath must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.MediaType), "TV series id MediaType must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeriesIdDataModel.OriginalLanguage), "TV series id OriginalLanguage must not be empty");
            Assert.NotEmpty(tvSeriesIdDataModel.GenreIds);
            Assert.True(tvSeriesIdDataModel.Popularity >= 0.0, "TV series id Popularity must not be a negative decimal number");
            Assert.True(tvSeriesIdDataModel.VoteAverage >= 0.0, "TV series id VoteAverage must not be a negative decimal number");
            Assert.True(tvSeriesIdDataModel.VoteCount >= 0, "TV series id VoteCount must not be a negative number");
        ...
        {  // TV Episode
            ...
            Assert.True(tvEpisodeIdDataModel.TmdbId > 0, "TV episode id TmdbId must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(tvEpisodeIdDataModel.Name), "TV episode id Name must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvEpisodeIdDataModel.Overview), "TV episode id Overview must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvEpisodeIdDataModel.MediaType), "TV episode id MediaType must not be empty");
            Assert.True(tvEpisodeIdDataModel.VoteAverage >= 0.0, "TV episode id VoteAverage must not be a negative decimal number");
            Assert.True(tvEpisodeIdDataModel.VoteCount >= 0, "TV episode id VoteCount must not be a negative number");
            Assert.True(tvEpisodeIdDataModel.EpisodeNumber > 0, "TV episode id EpisodeNumber must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(tvEpisodeIdDataModel.EpisodeType), "TV episode id EpisodeType must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvEpisodeIdDataModel.ProductionCode), "TV episode id ProductionCode must not be empty");
            Assert.True(tvEpisodeIdDataModel.Runtime > 0, "TV episode id Runtime must be a positive number");
            Assert.True(tvEpisodeIdDataModel.SeasonNumber > 0, "TV episode id SeasonNumber must be a positive number");
            Assert.True(tvEpisodeIdDataModel.ShowId > 0, "TV episode id ShowId must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(tvEpisodeIdDataModel.StillPath), "TV episode id StillPath must not be empty");
        ...
        {  // TV Season
            ...
            Assert.True(tvSeasonIdDataModel.TmdbId > 0, "TV season id TmdbId must be a positive number");
            Assert.False(String.IsNullOrWhiteSpace(tvSeasonIdDataModel.Name), "TV season id Name must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeasonIdDataModel.Overview), "TV season id Overview must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeasonIdDataModel.PosterPath), "TV season id PosterPath must not be empty");
            Assert.False(String.IsNullOrWhiteSpace(tvSeasonIdDataModel.MediaType), "TV season id MediaType must not be empty");
            Assert.True(tvSeasonIdDataModel.VoteAverage >= 0.0, "TV season id VoteAverage must not be a negative decimal number");
            Assert.True(tvSeasonIdDataModel.SeasonNumber > 0, "TV season id SeasonNumber must be a positive number");
            Assert.True(tvSeasonIdDataModel.ShowId > 0, "TV season id ShowId must be a positive number");
            Assert.True(tvSeasonIdDataModel.EpisodeCount > 0, "TV season id EpisodeCount must be a positive number");
